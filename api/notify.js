...
export default async (req, res) => {
  ...
  try {
    ...
    // [تحسين جذري] الكشف عن Webhook بطريقة أكثر دقة
    // YouCanPay يرسل دائماً كائن 'customer'. Stripe يرسل 'object: event'.
    // إذا لم تكن هذه موجودة، فهو طلب مباشر من موقعنا.
    ...
    // التحقق من الحقول المطلوبة (Validation Logic)
    // إذا كان الحقل ناقصاً، نقوم بتسجيل الخطأ بدقة لمعرفة المصدر
    if (!rawClientName || !rawClientEmail || !rawClientPhone || !rawInquiryId) {
       console.error("❌ Validation Failed. Payload:", JSON.stringify(data));
       throw new Error(`Missing required fields: ${!rawClientName?'clientName ':''}${!rawClientEmail?'clientEmail ':''}${!rawClientPhone?'clientPhone ':''}${!rawInquiryId?'inquiryId':''}`);
    }

    // التحقق من صحة الإيميل والهاتف
    if (rawClientEmail && !validateEmail(rawClientEmail)) throw new Error('Invalid email format');
    // if (!validatePhone(rawClientPhone)) throw new Error('Invalid phone number format');

    // [تحسين] معالجة الحالة (Status Handling)
    let finalStatus = 'pending';
    if (isWebhook) {
        // YouCanPay: 1 = Paid
        finalStatus = (data.status === 1 || data.status === 'paid') ? 'paid' : 'failed';
    } else {
        // Direct API
    ...
    const normalizedData = {
      ...
      cashplusCode: sanitizeString(data.cashplus?.code || data.cashplusCode || null),
